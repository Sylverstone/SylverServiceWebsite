...
const transformImage = (result : NodeListOf<Element>) =>
{
...
const cancel = (origineDisplay : string,canceller : HTMLElement) =>
{    
    const ListeDisplay = ["flex","block","grid","inline-flex","inline-block"];
    const asideFlottant = document.querySelector(`.fixe-asideFlottant`);
    if(!(asideFlottant instanceof HTMLElement)) return;
    ...
    if(asideFlottant instanceof HTMLElement && canceller instanceof HTMLElement)
    {
        ...
        {
            enfantUL.forEach(li => {if(li instanceof HTMLElement) li.style.display = `${origineDisplay}`});
